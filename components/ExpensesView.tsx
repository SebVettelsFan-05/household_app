...
import { BUYERS, type Expense, type ExpenseCategoryDef } from "@/lib/types";
...
type Props = {
  ...
  onCategoriesChange: (next: ExpenseCategoryDef[]) => void;
      ...
      {expenses.length > 0 ? (
        <section className="split-card">
          ...
          <div className="split-step">
            ...
            <span>
              Everyone deposits <strong>{fmtMoney(summary.share)}</strong> into
              the joint account.
            </span>
          </div>
          ...
          {summary.roundingRemainder !== 0 ? (
            <div className="split-note">
              Rounding leaves {fmtMoney(Math.abs(summary.roundingRemainder))}{" "}
              {summary.roundingRemainder > 0 ? "short of" : "over"} the total —
              one person can absorb it.
            </div>
          ) : null}
        </section>
      ) : null}
